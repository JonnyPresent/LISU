...
def save_checkpoint(params, is_best=False, directory='ckpt', filename=''):
...
def reverse_one_hot(image):
    ...
    # Returns
        ...
        class key.
    """
    image = image.permute(0, 2, 3, 1)  # torch.Size([1, 240, 320, 14])
    x = torch.argmax(image, dim=-1)  # torch.Size([1, 240, 320])
...
def label_accuracy_score(label_trues, label_preds, n_class):   # [h*w, h*w] , [.,.]
