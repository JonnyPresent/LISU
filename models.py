...
#         super().__init__()
#         self.in_channels = 4
#         self.channels = [32, 64, 128, 256, 256]
#
#         # Encoder layers
...
#         # 256 x 15 x 20
#
#         xmid_r = torch.cat((self.mid_r(x5), x5), dim=1)  # 256*2 512
#         xmid_s = torch.cat((self.mid_s(x5), x5), dim=1)
#         # 512 x 15 x 20
#
#         x4d_r = self.deconv4_r(xmid_r)  # 256
#         x4d_s = self.deconv4_s(xmid_s)  # 256
#
#         # #a添加mask分割？
#         x3d_r = self.deconv3_r(torch.cat([x4d_r] + [x4d_s] + [x4], dim=1))  # 768 -- 128
